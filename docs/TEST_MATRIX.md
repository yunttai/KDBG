...
| Optional MemProcFS runtime | BLOCKED | `vmm.dll` load failed with Win32 error 126 |
...
| Final physical write | BLOCKED | `NOT_RUN`; DADF transaction is interim only |
| Process write/Freeze | BLOCKED | no final live write evidence |
| Driver Verifier | BLOCKED | `NOT_RUN` |
| Final v2 | BLOCKED | scoped GIF is not hash-bound final-package v2 |
