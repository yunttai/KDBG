...
std::pair<Metric, Metric> BenchmarkScannerAndNextScan() {
    ...
    const auto storage_estimate = CandidateStorageEstimate(scanner);
    first_metric.pass = first.Value().result_count == expected &&
        !first.Value().truncated;
    first_metric.Add("bytes_scanned", first.Value().bytes_scanned);
    first_metric.Add("result_count", static_cast<std::uint64_t>(
        first.Value().result_count));
    first_metric.Add("elapsed_ms", Milliseconds(first_elapsed));
    first_metric.Add("throughput_mib_s", MebibytesPerSecond(
        first.Value().bytes_scanned, first_elapsed));
    ...
    next_metric.pass = next.Value().result_count == expected / 2U;
    next_metric.Add("input_candidates", static_cast<std::uint64_t>(expected));
    next_metric.Add("result_count", static_cast<std::uint64_t>(
        next.Value().result_count));
    next_metric.Add("elapsed_ms", Milliseconds(next_elapsed));
    return {std::move(first_metric), std::move(next_metric)};
}
...
Metric BenchmarkPointerCap() {
    ...
    constexpr std::size_t kCap = 128U;
    ...
    metric.pass = result.Value().size() == kCap;
    metric.Add("configured_cap", static_cast<std::uint64_t>(kCap));
    metric.Add("result_count", static_cast<std::uint64_t>(result.Value().size()));
    metric.Add("cap_honored", result.Value().size() <= kCap);
    metric.Add("elapsed_ms", Milliseconds(elapsed));
    return metric;
}
...
Metric BenchmarkSnapshot() {
    ...
    const bool integrity = loaded.Value().Bytes().size() == kSize &&
        loaded.Value().Checksum() == captured.Value().Checksum() &&
        loaded.Value().Bytes() == captured.Value().Bytes();
    metric.pass = integrity && removed && !remove_error;
    metric.Add("bytes", static_cast<std::uint64_t>(kSize));
    metric.Add("capture_ms", Milliseconds(capture_elapsed));
    metric.Add("capture_mib_s", MebibytesPerSecond(kSize, capture_elapsed));
    metric.Add("save_ms", Milliseconds(save_elapsed));
    metric.Add("save_mib_s", MebibytesPerSecond(kSize, save_elapsed));
    metric.Add("load_ms", Milliseconds(load_elapsed));
    metric.Add("load_mib_s", MebibytesPerSecond(kSize, load_elapsed));
...
Metric BenchmarkCancellation() {
    ...
    const bool cancelled = result.has_value() && !result->Ok() &&
        result->GetError().code == kdbg::ErrorCode::Cancelled;
    metric.pass = cancelled;
    metric.Add("cancelled", cancelled);
    metric.Add("request_to_completion_ms", Milliseconds(
        completed - cancellation_requested));
    metric.Add("worker_total_ms", Milliseconds(completed - worker_started));
    if (result && !result->Ok()) {
        metric.Add("error_code", static_cast<std::uint64_t>(
